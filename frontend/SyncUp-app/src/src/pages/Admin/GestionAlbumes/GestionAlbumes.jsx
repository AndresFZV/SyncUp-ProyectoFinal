...
const GestionAlbumes = () => {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      console.log('📀 Álbumes cargados:', albumesData);
  ...
  const handleClickAlbum = async (album) => {
    ...
    const idAlbum = album.id || album._id || album.albumId;
  ...
  const handleEditar = (album, e) => {
    ...
    setFormData({
      ...
      artistaId: album.artistaId || album.artistId || '',
      imagen: null
    });
  ...
  const handleEliminar = async (album, e) => {
    e.stopPropagation();
    
    const albumId = album.id || album._id;
    
    const confirmacion = window.confirm(
      `¿Estás seguro de eliminar el álbum "${album.nombre}"?\n\n` +
    ...
    try {
      ...
      cargarDatos();
      alert(`Álbum "${album.nombre}" eliminado exitosamente`);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (albumEditando) {
        const albumId = albumEditando.id || albumEditando._id;
  ...
  const albumesFiltrados = albumes.filter(a => 
    ...
    a.artistaNombre?.toLowerCase().includes(busqueda.toLowerCase())
  );

  const obtenerNombreArtista = (album) => {
    // Primero intentar usar el nombre que viene del DTO del backend
    if (album && album.artistaNombre) {
      return album.artistaNombre;
    }
    
    // Fallback: buscar en la lista de artistas
    if (album && album.artistaId) {
      const artista = artistas.find(a => a.artistId === album.artistaId);
  ...
  return (
    <div className="gestion-container">
      ...
      <div className="search-bar">
        ...
        <input
          type="text"
          placeholder="Buscar por nombre, descripción o artista..."
          value={busqueda}
      ...
      <div className="albums-grid">
        {albumesFiltrados.map((album) => {
          const albumKey = album.id || album._id;
          ...
          return (
            ...
            >
              ...
              <div className="album-info">
                <h3>{album.nombre}</h3>
                <p className="album-artist">{obtenerNombreArtista(album)}</p>
                <p className="album-description">{album.descripcion}</p>
                <div className="album-actions">
                  <button className="btn-edit" onClick={(e) => handleEditar(album, e)}>
                    <FaEdit />
                  </button>
                  <button className="btn-delete" onClick={(e) => handleEliminar(album, e)}>
                    <FaTrash />
                  </button>
                </div>
              </div>
      ...
      {/* Modal de Canciones - SIN CAMBIOS */}
            ...
            <div className="modal-header-album">
              <div className="album-info-header">
                ...
                <div>
                  ...
                  <p className="artist-name-modal">{obtenerNombreArtista(albumSeleccionado)}</p>
            ...
            {cancionReproduciendo && (
              <div className="audio-player">
                ...
                <div className="player-info-section">
                  ...
                  <p>{cancionReproduciendo.artistaNombre || obtenerNombreArtista(albumSeleccionado)}</p>
