...
const GestionCanciones = () => {
  ...
  const cargarDatos = async () => {
    setLoading(true);
    try {
      const [cancionesData, artistasData, albumesData] = await Promise.all([
        getAllCanciones(),
        getAllArtistas(),
        getAllAlbumes()
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Validaciones
    const validaciones = [
      validateRequired(formData.titulo, 'Título'),
      validateRequired(formData.genero, 'Género'),
      validateYear(formData.anio),
    ...
    try {
      ...
      alert(error.message || 'Error al guardar la canción');
    } finally {
      setLoading(false);
    }
  };
                ...
                <td>
                  {cancion.imagenUrl ? (
                    <img 
                      ...
                      alt={cancion.titulo} 
                      className="cancion-thumbnail"
                    />
                  ) : (
                    <div className="cancion-thumbnail-placeholder">
                      <FaMusic />
                    </div>
                  )}
                </td>
