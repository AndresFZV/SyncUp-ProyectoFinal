...
import { cargaMasivaCanciones } from '../../../services/cancionesService';
import { cargaMasivaAlbum } from '../../../services/albumesService';
...
const CargaMasiva = () => {
  ...
  const handleCargarCanciones = async () => {
    ...
    try {
      const data = await cargaMasivaCanciones(archivoTxt, archivoZip);
      setResultado(data);
      setArchivoTxt(null);
      setArchivoZip(null);
      // Reset file inputs
      ...
      setErrores([error.message || 'Error al procesar los archivos']);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCargarAlbum = async () => {
    ...
    try {
      const data = await cargaMasivaAlbum(archivoTxt, imagenAlbum, archivoZip);
      setResultado(data);
      setArchivoTxt(null);
      setArchivoZip(null);
      setImagenAlbum(null);
      ...
      console.error('Error:', error);
      setErrores([error.message || 'Error al procesar el archivo']);
    } finally {
      setLoading(false);
    }
  };
